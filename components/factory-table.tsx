...
type Props = { title: string; eyebrow: string; description: string; table: string; columns: string[]; limit?: number }
...
export default function FactoryTable({ title, eyebrow, description, table, columns, limit = 100 }: Props) {
  ...
  return <main className="shell">
    ...
    <section className="hero" style={{ gridTemplateColumns: '1fr' }}><div><p className="eyebrow">{eyebrow}</p><h1 style={{ fontSize: 'clamp(40px,6vw,64px)' }}>{title}</h1><p className="lede">{description}</p></div></section>
