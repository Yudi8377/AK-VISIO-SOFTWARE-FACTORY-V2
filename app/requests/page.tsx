...
export default function Requests(){
  ...
  useEffect(()=>{load()},[])
  async function save(){if(!text.trim())return; setBusy(true);setMessage(''); const {error}=await supabase.from('factory_requests').insert({request_text:text.trim(),status:'DRAFT'}); setBusy(false); if(error)setMessage(error.message);else{setText('');setMessage('Request saved');load()}}
